...
export default function Stepper({ steps = [], orientation = 'horizontal' ,onFinish}: StepperProps) {
  ...
  return (
    <div className={cn(
      ...
      <div className={cn(
        "flex mb-8 flex-1",
        orientation === 'vertical' ? 'flex-col space-y-40 justify-center items-center' : 'flex-row space-x-4'
      )}>
        {steps.map((step, index) => (
          <div
            ...
            className={cn(
              "flex items-center",
              orientation === 'vertical' ? 'flex-row space-x-4' : 'flex-col space-y-2'
            )}
          >
            <div
              className={cn(
