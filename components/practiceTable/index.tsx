...
import { PracticeSession, practiceSessionsAPI } from '@/db/practiceSession/practiceSession.client';
...
export default function QuestionTable({questions,activeTab}:{questions:IQuestions[],activeTab:string}) {
 
  
  const router = useRouter();


  const [inProgressSessions,setInProgressSessions] = useState<PracticeSession[]>([])
  const [inProgressSessionIndex,setInProgressSessionIndex] = useState<Record<string,string>>({})
  const [completedSessionIndex,setCompletedSessionIndex] = useState<Record<string,string>>({})
  const [filteredQuestions, setFilteredQuestions] = useState<IQuestions[]>(questions);
  ...
  const handleCreateSession = async (e:React.MouseEvent<HTMLButtonElement, MouseEvent>,question:IQuestions)=>{
    e.preventDefault(); // Prevent default button behavior

  // Languages are hard coded for now
   const sessionId = await practiceSessionsAPI.startSession(question.key,'react',question.content)
  ...
  const updateFilteredQuestions = (inProgress: Record<string, string>, completed: Record<string, string>) => {
      ...
      <TableBody>
        {questions.map((question, index) => (
          <TableRow key={question.key}>
            ...
            <TableCell className="hidden md:table-cell"><TagsCell tags={question.tags}/></TableCell>
            <TableCell className="hidden md:table-cell">{question.author}</TableCell>
            {/* <TableCell className="hidden md:table-cell">{product.createdAt}</TableCell> */}
            <TableCell className='w-8'>
              {/* <DropdownMenu>
                <DropdownMenuTrigger asChild> */}
                {!isLoading ? !inProgressSessionIndex[question.key] ? <Button
                    type='button'
                    variant={"default"}
                    size={"sm"}
                    className='w-full'
                    onClick={(e)=>{
                      if(completedSessionIndex[question.key]){
                        router.push(`/lab/${completedSessionIndex[question.key]}`)
                      }else {
                  ...
                  >
                    ...
                    {!completedSessionIndex[question.key] ? "Start" : "Re-Attempt" } 
                  </Button> : <Button
                   type='button'
                   variant={"default"}
                   size={"sm"}
                   className='w-full'
                   onClick={()=>{
                    router.push(`/lab/${inProgressSessionIndex[question.key]}`);
                   }}
                   >
